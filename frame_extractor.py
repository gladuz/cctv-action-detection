...
videos = open('processed_data/explore_error_files.txt', 'r').read().split('\n')
videos = [videos.replace(".xml", "_resized.mp4") for videos in videos]
videos = [videos.replace("xmls", "resized") for videos in videos]
print(videos)
error_files = set()
...
for video_file in tqdm(videos, desc='Extracting videos'):
    ...
    if video_file in error_files:
        continue
    #os.removedirs(os.path.join(SAVE_PATH, frames_folder_name))
